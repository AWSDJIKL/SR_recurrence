...
from model import ELAB


def make_model(args):
    print("prepare model")
    if args.is_PMG:
        print("SwinIR use PMG")
    else:
        print("SwinIR")
    return SwinIR(args)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False, ape=False):
...
class RSTB(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 img_size=224, patch_size=4, resi_connection='1conv', ape=False):
...
class PatchEmbed(nn.Module):
    ...
    将输入图片按照patch大小展开，展开后的tensor size为 (B,N,C), N为patch的数量，
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        # patch的分辨率，关系到RSTB的设置
        # patch分辨率指的是以一个patch大小代表一个像素点，输入图片的分辨率（高和宽）
        patches_resolution = [img_size[0] // patch_size[0], img_size[1] // patch_size[1]]
        self.img_size = img_size
        self.patch_size = patch_size
        self.patches_resolution = patches_resolution
        # patch分辨率高和宽相乘得到一个batch中一个通道的图片包含多少个patch
...
#         # self.ca_conv = nn.Sequential(nn.Conv2d(embed_dim, embed_dim // 4, 1, bias=False),
#         #                              nn.ReLU(),
...
#
#         # # channel attention
#         # c_avg_x = self.avg_pool(x)
#         # c_max_x = self.max_pool(x)
#         # c_x = c_avg_x + c_max_x
#         # c_x = self.ca_conv(c_x)
#         # c_x = self.relu(c_x)
#         # x = c_x * x
#         # # spatial attention
#         # s_avg_x = torch.mean(x, 1, keepdim=True)
#         # s_max_x, _ = torch.max(x, 1, keepdim=True)
#         # s_x = self.relu(self.sa_conv(torch.cat([s_avg_x, s_max_x], 1)))
#         # # print(s_x.shape)
#         # x = s_x * x
...
class SwinIR(nn.Module):
    ...
    def __init__(self, args, img_size=384, patch_size=1, in_chans=3,
        ...
        if self.ape:
            ...
            # 8 40
            # 16 20
            # self.adaptive_position_embedding = AdaptivePositionEmbedding(embed_dim, 4, 10)

            # self.adaptive_position_embedding = nn.ModuleList(
            #     [AdaptivePositionEmbedding(embed_dim, num_freq=16 * (i + 1), max_freq=20 * (i + 1)) for i in
    ...
    def check_image_size(self, x):
        '''
        检查输入图像的大小，如果不能被window size整除，则进行padding。padding采用reflect方式。
        Args:
            x: 输入图像
    ...
    def forward_features(self, x, step=-1):
        ...
        # if self.ape:
        #     pos_emb = x
        #     pos_emb = self.adaptive_position_embedding(pos_emb, x_size[0], x_size[1])
        #     x = x + pos_emb
        x = self.pos_drop(x)
    ...
    def forward(self, x, step=-1):
        H, W = x.shape[2:]
        # 首先检查输入图像大小，如果不能被window size整除，则进行padding
...
if __name__ == '__main__':
    ...
    model = SwinIR(args, upscale=4, img_size=(height, width),
