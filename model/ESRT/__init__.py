...
def make_model(args):
    ...
    if args.is_PMG:
        ...
        print("ESRT")
    return ESRT(args, upscale=args.scale)
...
class Un(nn.Module):
    def __init__(self, n_feats, wn):
        ...
        self.weight2 = common.Scale(1)
        self.weight1 = common.Scale(1)
        # transformer部分，后续可考虑替换为vison_lstm
...
class ESRT(nn.Module):
    def __init__(self, args, upscale=4, conv=common.default_conv):
        ...
        channel_adaptive = [
            nn.Conv2d(n_feats * i, n_feats, kernel_size, padding=1, stride=1)
            for i in self.part
        ]
        self.reduce = nn.Sequential(*channel_adaptive)

    def forward(self, x1, step=-1):
        # x1 = self.sub_mean(x1)
        x1 = self.head(x1)
        res2 = x1
        # res2 = x2
