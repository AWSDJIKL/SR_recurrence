...
class BCRN(nn.Module):
    def __init__(self, args):
        ...
        self.body = nn.Sequential(*[BluePrintShortcutBlock(64, 64, 3) for _ in range(args.part[-1])])
        ...
        self.conv2 = nn.Sequential(*[
            blueprint_conv_layer(64 * i, 64, 3) for i in args.part
        ])
    ...
    def forward(self, x, step=-1):
        ...
        for i in range(self.part[step]):
            x = self.body[i](x)
            out_c.append(x)
        out_C = self.activation(self.conv2[step](torch.cat(out_c, dim=1)))
