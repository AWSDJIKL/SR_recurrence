...
'''
# @Time    : 2024/8/14 23:11
# @Author  : LINYANZHEN
# @File    : vision_lstm_util.py
import collections
import itertools
import math
...
class SequenceConv2d(nn.Conv2d):
    ...
    def forward(self, x):
        ...
        if self.seqlens is None:
            # 出于未知原因，这里要求h是一个完全平方数，测试中发现去掉这一限制并未影响代码运行
...
class VitPatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        # 过一个卷积，做一个channel维度变化，然后reshape，将channel维度移动到最后一维
        x = self.proj(x)
