...
from model.BCRN import BluePrintShortcutBlock
import os
...
def make_model(args):
    ...
    if args.is_PMG:
        ...
        print("HAT")
    return HAT(args, upscale=args.scale)
...
class HAT(nn.Module):
    ...
    def __init__(self,
        ...
        if self.ape:
            ...
            # self.adaptive_position_embedding = nn.ModuleList(
            #     [AdaptivePositionEmbedding(embed_dim, 4 * (i + 1), 10 * (i + 1)) for i in
            #      range(self.num_layers)])
            # [1,3,5,7]
            # [1,3,5,7]
            # kernel_size_dilation = [1, 1, 3, 3] #31.73
            # self.adaptive_position_embedding = nn.ModuleList(
            #     [AdaptivePositionEmbedding(embed_dim, kernel_size=k_d, dilation=k_d) for k_d in kernel_size_dilation])

            # self.adaptive_position_embedding = nn.ModuleList(
            #     [PosCNN(embed_dim,embed_dim) for _ in range(self.num_layers)])

            # self.absolute_pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dim))
            # trunc_normal_(self.absolute_pos_embed, std=.02)
            # self.absolute_pos_embed = nn.Parameter(torch.zeros(1, 1024, embed_dim))
            # trunc_normal_(self.absolute_pos_embed, std=.02)
            # self.adaptive_position_embedding = AdaptivePositionEmbedding(embed_dim)
            # self.pos_h = 32
            # self.pos_w = 32
    ...
    def forward_features(self, x, step=-1):
        ...
        for i in range(self.part[step]):
            ...
            # cnn
            # if self.ape:  #31.73
            #     x = self.adaptive_position_embedding[i](x, x_size[0], x_size[1])

        # x = self.layers[-1](x, x_size, params)
